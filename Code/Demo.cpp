...
void Demo::prepare_uniform_buffer()
{
	...
	glm::vec3 eye = { 0.0f, 2.0f, 4.0f };		// The position of the camera
...
void Demo::prepare_texture(char* file)
{
...
void Demo::prepare_vb_ib(char* file)
{
	// Part 1
	...
	uint32_t vertexArraySize = vertices.size() * sizeof(VertexStructure);
	...
	uint32_t indexArraySize = indices.size() * sizeof(uint16_t);
...
void Demo::build_secondary_cmd()
{
	...
	vkCmdBindIndexBuffer(cmd, indexDataGPU->buffer, 0, VK_INDEX_TYPE_UINT16);

	// Draw the indexed triangle
	// We have 36 indices in the index buffer
	// We are drawing these 36 indices one time
	vkCmdDrawIndexed(cmd, numIndices, 1, 0, 0, 1);
...
void Demo::prepare()
{
	...
	if (firstInit)
	{
		...
		prepare_vb_ib("../../../Assets/kitten.3Dobj");
		...
		// load the texture that gets sent to 
		...
		prepare_texture("../../../Assets/CatColor.png");
